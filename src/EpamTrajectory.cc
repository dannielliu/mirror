...
// ********************************************************************
//
/// \file optical/Epam/src/EpamTrajectory.cc
/// \brief Implementation of the EpamTrajectory class
//
//
//
...
#include "EpamTrajectoryPoint.hh"
...
G4Allocator<EpamTrajectory> EpamTrajectoryAllocator;

EpamTrajectory::EpamTrajectory()
...
EpamTrajectory::EpamTrajectory(const G4Track* aTrack)
{
    ...
    fpPointsContainer = new EpamTrajectoryPointContainer();
    // Following is for the first trajectory point
    fpPointsContainer->push_back(new EpamTrajectoryPoint(aTrack));
...
EpamTrajectory::EpamTrajectory(EpamTrajectory & right) : G4VTrajectory()
{
    ...
    for(size_t i=0;i<right.fpPointsContainer->size();++i) {
        EpamTrajectoryPoint* rightPoint
            = (EpamTrajectoryPoint*)((*(right.fpPointsContainer))[i]);
        fpPointsContainer->push_back(new EpamTrajectoryPoint(*rightPoint));
    }
...
void EpamTrajectory::ShowTrajectory(std::ostream& os) const
{
...
void EpamTrajectory::DrawTrajectory() const
{
  // Invoke the default implementation in G4VTrajectory...
...
void EpamTrajectory::DrawTrajectory(G4int i_mode) const
{
    // Invoke the default implementation in G4VTrajectory...
...
void EpamTrajectory::AppendStep(const G4Step* aStep)
{
    fpPointsContainer->push_back(new EpamTrajectoryPoint(aStep));
}

G4ParticleDefinition* EpamTrajectory::GetParticleDefinition()
{
    return (G4ParticleTable::GetParticleTable()->FindParticle(ParticleName));
}

void EpamTrajectory::MergeTrajectory(G4VTrajectory* secondTrajectory)
{
    if(!secondTrajectory) return;

    EpamTrajectory* second = (EpamTrajectory*)secondTrajectory;
...
const std::map<G4String,G4AttDef>* EpamTrajectory::GetAttDefs() const
{
    G4bool isNew;
...
std::vector<G4AttValue>* EpamTrajectory::CreateAttValues() const
{
