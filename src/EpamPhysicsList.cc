...
// ********************************************************************
//
/// \file optical/Epam/src/EpamPhysicsList.cc
/// \brief Implementation of the EpamPhysicsList class
//
//
//
...
EpamPhysicsList::EpamPhysicsList(G4String physName) : G4VModularPhysicsList()
{
    ...
    if (factory.IsReferencePhysList(physName)) {
       ...
       if(!phys)G4Exception("EpamPhysicsList::EpamPhysicsList","InvalidSetup",
                            FatalException,"PhysicsList does not exist");
       fMessenger = new EpamPhysicsListMessenger(this);
    }
    ...
    physicsVector->push_back(new EpamExtraPhysics());
    physicsVector->push_back(opticalPhysics = new EpamOpticalPhysics(AbsorptionOn));

    physicsVector->push_back(new G4RadioactiveDecayPhysics());

    stepMaxProcess = new EpamStepMax();
}
...
void EpamPhysicsList::ClearPhysics()
{
...
void EpamPhysicsList::RemoveFromPhysicsList(const G4String& name)
{
    ...
    if (!success) {
       ...
       G4Exception("example EpamPhysicsList::RemoveFromPhysicsList()",
       "ExamEpamPhysicsList01",FatalException,message);
    }
}

void EpamPhysicsList::SetAbsorption(G4bool toggle)
{
       AbsorptionOn = toggle;
       RemoveFromPhysicsList("Optical");
       physicsVector->push_back(opticalPhysics = new EpamOpticalPhysics(toggle));
...
{
    if (verboseLevel >0) {
        G4cout << "EpamPhysicsList::SetCuts:";
...
void EpamPhysicsList::SetCutForGamma(G4double cut)
{
    fCutForGamma = cut;
    SetParticleCuts(fCutForGamma, G4Gamma::Gamma());
}

void EpamPhysicsList::SetCutForElectron(G4double cut)
{
    fCutForElectron = cut;
    SetParticleCuts(fCutForElectron, G4Electron::Electron());
}

void EpamPhysicsList::SetCutForPositron(G4double cut)
{
    fCutForPositron = cut;
    SetParticleCuts(fCutForPositron, G4Positron::Positron());
}

void EpamPhysicsList::SetStepMax(G4double step)
{
  MaxChargedStep = step;
  stepMaxProcess->SetStepMax(MaxChargedStep);
}

EpamStepMax* EpamPhysicsList::GetStepMaxProcess()
{
  return stepMaxProcess;
}

void EpamPhysicsList::AddStepMax()
{
...
void EpamPhysicsList::SetNbOfPhotonsCerenkov(G4int MaxNumber)
{
   opticalPhysics->SetNbOfPhotonsCerenkov(MaxNumber);
}

void EpamPhysicsList::SetVerbose(G4int verbose)
{
